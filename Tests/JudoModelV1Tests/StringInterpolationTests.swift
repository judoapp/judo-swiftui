...
class StringInterpolationTests: XCTestCase {
    ...
    func test_evaluatingExpressions_givenDataURLParametersUserInfoAndNoInterpolation_returnsOriginalString() throws {
        ...
        let sut = try makeSUT(nonInterpolatedString(), data: data, urlParameters: urlParameters, userInfo: userInfo)
        XCTAssertEqual(sut, nonInterpolatedString())
    }
    ...
    func test_evaluatingExpressions_incompleteInterpolation_returnsOriginalString() throws {
        ...
        let sut = try makeSUT("{{user.userid", userInfo: userInfo)
    ...
    func test_evaluatingExpressions_incorrectInterpolationWithTwoValues_throwsError() throws {
        ...
        let expectedError = StringExpressionError("Invalid expression \"user.userid user.name\"")
        throwsErrorWhenUnableToInterpolate("{{user.userid user.name}}", userInfo: userInfo, expectedError: expectedError)
    }
    ...
    func test_evaluatingExpressions_withInterpolationForUserInfo_returnsExpectedString() throws {
        ...
        let sut = try makeSUT("{{user.name}}", userInfo: userInfo)
    ...
    func test_evaluatingExpressions_withInterpolationForURLParameters_returnsExpectedString() throws {
        ...
        let sut = try makeSUT("{{url.page}}", urlParameters: urlParameters)
    ...
    func test_evaluatingExpressions_withMultipleInterpolations_returnsExpectedString() throws {
        ...
        let sut = try makeSUT(string, data: data, urlParameters: urlParameters, userInfo: userInfo)
    ...
    func test_evaluatingExpressions_uppercase_returnsExpectedString() throws {
        ...
        let sut = try makeSUT(string, urlParameters: urlParameters)
    ...
    func test_evaluatingExpressions_replaces_returnsExpectedString() throws {
        ...
        let sut = try makeSUT(string, userInfo: userInfo)
    ...
    func test_evaluatingExpressions_dateFormat_returnsExpectedString() throws {
        ...
        let urlParameters = constructURLParameters(with: ["time": "2022-02-01 19:46:31+0000"])
        let userInfo = constructUserInfo(with: ["day": "2022-02-01 19:46:31+0000"])
    ...
    func test_evaluatingExpressions_numberFormatNonNumber_throwsError() throws {
        ...
        throwsErrorWhenUnableToInterpolate(string, userInfo: userInfo, expectedError: .invalidArgument)
    }
    ...
    func test_evaluatingExpressions_numberFormatNone_returnsExpectedString() throws {
        ...
        let sut = try makeSUT(string, data: data, userInfo: userInfo)
    ...
    func test_evaluatingExpressions_multipleHelpersWithQuotationMarksInUserInfo_throwsError() throws {
        ...
        throwsErrorWhenUnableToInterpolate(string, userInfo: userInfo, expectedError: .argumentCount(3))
    }
    ...
    private func constructUserInfo(with parameters: [String: String]) -> UserInfo {
        parameters.reduce(into: UserInfo()) { userInfo, entry in
            userInfo[entry.key] = entry.value
        }
    }

    private func constructURLParameters(with parameters: [String: String]) -> UserInfo {
        parameters.reduce(into: UserInfo()) { userInfo, entry in
            userInfo[entry.key] = entry.value
        }
    }
