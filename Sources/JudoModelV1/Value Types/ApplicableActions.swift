...
///
/// Provides a means for doing a "lowest common denominator" approach to establishing agreement between multiple actors (such as multiple selected nodes) what actions maybe legally performed. This other than nodes -- such as current pasteboard contents -- may participate.
public struct ApplicableActions: OptionSet {
    ...
    public static let setInitialScreen = ApplicableActions(rawValue: 1 << 7)
...
extension ApplicableActions: CustomDebugStringConvertible {
    public var debugDescription: String {
        ...
        if self.contains(.all) {
            ...
            options += ["lock"]
        } else if self.contains(.setInitialScreen) {
            options += ["setInitialScreen"]
        }
...
extension Swift.Collection where Element: Node {
    public var applicableActions: ApplicableActions {
        ...
        return allActions.reduce(.all) { (result, nodeApplicableActions) in
            ApplicableActions(rawValue: result.rawValue & nodeApplicableActions.rawValue)
        }
    }
}
