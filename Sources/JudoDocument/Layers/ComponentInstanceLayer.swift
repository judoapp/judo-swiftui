...
public struct ComponentInstanceLayer: Layer {
    public var id: UUID
    ...
    public init(from decoder: Decoder) throws {
        ...
        overrides = try overridesContainer.allKeys.reduce(into: [:]) { partialResult, key in
            ...
            case .video:
                ...
                switch meta.version {
                case ..<21:
                    let url = try nestedContainer.decode(String.self, forKey: ._0)
                    let value = Variable(Video(url: url))
                    partialResult[key.stringValue] = .video(value)
                default:
                    let value = try nestedContainer.decode(Variable<Video>.self, forKey: ._0)
                    partialResult[key.stringValue] = .video(value)
                }
            case .number:
