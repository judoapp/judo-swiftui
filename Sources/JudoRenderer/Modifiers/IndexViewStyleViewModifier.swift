...
struct IndexViewStyleViewModifier: SwiftUI.ViewModifier {
    @ObservedObject var modifier: IndexViewStyleModifier

    func body(content: Content) -> some SwiftUI.View {
        switch modifier.indexViewStyle {
        case .page(let backgroundDisplayMode):
            switch backgroundDisplayMode {
            case .automatic:
                content.indexViewStyle(.page(backgroundDisplayMode: .automatic))
            case .always:
