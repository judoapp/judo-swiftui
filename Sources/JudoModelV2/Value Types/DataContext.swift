...
public struct DataContext: Hashable {
    ...
    public mutating func add(dataSource: DataSource, collections: [CollectionLayer]) {
    ...
    public func adding(dataSource: DataSource, collections: [CollectionLayer]) -> DataContext {
        var dataContext = self
        dataContext.add(dataSource: dataSource, collections: collections)
        return dataContext
    }
    
    public func publisher(properties: MainComponent.Properties, simulateSlowNetwork: Bool = false) -> AnyPublisher<Result<Any?, Error>, Never> {
        ...
        func publisher(for request: Request, data: Any?) -> AnyPublisher<Result<Any?, Error>, Never> {
            ...
            urlRequest.httpBody = try? request.httpBody?
                .evaluatingExpressions(data: data, properties: properties)
                .data(using: .utf8)
            
            urlRequest.allHTTPHeaderFields = request.headers.reduce(nil) { result, header in
                guard let value = try? header.value.evaluatingExpressions(data: data, properties: properties) else {
                    return result
                }
    ...
    public static func dataContext(for node: Node) -> DataContext {
        ...
        var collectionPath = [CollectionLayer]()
