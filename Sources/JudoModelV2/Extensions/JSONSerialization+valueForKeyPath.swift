...
extension JSONSerialization {
    ...
    /// context encompasses JSON data made available from a `DataSource`. The `data` parameter is expected to be a
    ...
    public static func value(forKeyPath keyPath: String, data: Any?, properties: MainComponent.Properties) -> Any? {
...
private func fetchValueByKeyPath(dict: [String: Any], keyPath: String) -> Any? {
    ...
    let match = possibilities.last { entry in
    ...
    guard let nestedDictionary = dict[match.key] as? [String: Any] else {
        os_log(.error, "Invalid nested keypath into a non-dictionary value: %s", match.remainder)
        return nil
    }
