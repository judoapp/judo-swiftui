...
enum Formatters {
    ...
    static let customizableNumberFormatter = CustomizableNumberFormatter()

    static let defaultDateCreator = ISO8601DateFormatter()

    // Use the local variant when the date string omits time zone
    static let localDateCreator: ISO8601DateFormatter = {
...
extension String {
    public func evaluatingExpressions(data: Any?, properties: MainComponent.Properties) throws -> String {
...
struct ExpressionEvaluator {
    ...
    private let expressionHelper: ExpressionHelper
    ...
    init(
        ...
        localDateCreator: ISO8601DateFormatter = Formatters.localDateCreator
    ) {
        expressionHelper = ExpressionHelper(
            ...
            localDateCreator: localDateCreator
        )
    }

    func evaluate(string: String) throws -> String {
        var string = string
        ...
        while let match = nextMatch(in: string) {
            let outerRange = Range(match.range(at: 0), in: string)!
            let innerRange = Range(match.range(at: 1), in: string)!
            let expression = String(string[innerRange])
            ...
            string.replaceSubrange(outerRange, with: newResult)
        }
        return string
    }
}
...
private struct ExpressionHelper {
    ...
    private let properties: MainComponent.Properties
    ...
    init(
        ...
        self.localDateCreator = localDateCreator
    }
    ...
    /// Checks the data, and helpers for the passed keyPath.
    ...
    private func stringValue(keyPathOrStringLiteral: String) throws -> String? {
        ...
        case let float as CGFloat:
            return customizableNumberFormatter.formatString(float as NSNumber, using: .none)
    ...
    /// Similar to `stringValue` this checks the data or the passed StringLiteral
    /// to see if it can be converted into a NSNumber.
    private func numberValue(keyPathOrStringLiteral: String) throws -> NSNumber? {
        // Check if keyPathOrStringLiteral starts and ends with "", if so remove them and return it,
        // this allows for string literals to be parsed.
        if keyPathOrStringLiteral.isEnclosedInQuotes {
            let updatedString = keyPathOrStringLiteral.removeQuotationMarks()
...
class CustomizableNumberFormatter: NumberFormatter {
