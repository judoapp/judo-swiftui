    ...
    public enum Override {
        public enum Value: Hashable, ExpressibleByStringLiteral, ExpressibleByFloatLiteral, ExpressibleByBooleanLiteral, ExpressibleByIntegerLiteral {
            ...
            public init(floatLiteral value: FloatLiteralType) {
                self = .number(NumberValue(floatLiteral: value))
            }

            public init(integerLiteral value: IntegerLiteralType) {
                self = .number(NumberValue(floatLiteral: Double(value)))
            }

            public init(booleanLiteral value: BooleanLiteralType) {
                self = .boolean(BooleanValue(booleanLiteral: value))
            }
        }
    }
        ...
        overrides = try overridesContainer.allKeys.reduce(into: [:]) { partialResult, key in
            ...
            case .number:
                ...
                case ..<14:
                    ...
                    let value = NumberValue(floatLiteral: floatLiteral)
                    partialResult[key.stringValue] = .number(value)
                default:
                    let value = try nestedContainer.decode(NumberValue.self, forKey: ._0)
                    partialResult[key.stringValue] = .number(value)
                }
            case .boolean:
                ...
                case ..<14:
                    ...
                    let value = BooleanValue(booleanLiteral: booleanLiteral)
                    partialResult[key.stringValue] = .boolean(value)
                default:
                    let value = try nestedContainer.decode(BooleanValue.self, forKey: ._0)
                    partialResult[key.stringValue] = .boolean(value)
                }
            case .image:
